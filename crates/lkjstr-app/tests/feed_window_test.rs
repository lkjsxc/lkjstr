use lkjstr_app::{
    FeedWindowEvidence, FeedWindowFlags, FeedWindowStatus, NewestCursorPolicy, TopAnchorAction,
    TopAnchorInput, decide_top_anchor, empty_feed_window, feed_window_empty_ready,
    reduce_feed_window,
};
...
#[test]
fn top_anchor_keeps_live_prepends_visible_at_top() {
...
#[test]
fn top_anchor_preserves_visible_row_away_from_top() {
    let decision = decide_top_anchor(TopAnchorInput {
...
#[test]
fn top_anchor_does_not_lock_while_restoring_or_pruned() {
    for input in [
        TopAnchorInput {
            scroll_offset_px: 0,
            top_tolerance_px: 2,
