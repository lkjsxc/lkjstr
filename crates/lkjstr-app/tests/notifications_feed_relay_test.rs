...
#[test]
fn notifications_feed_relay_progressive_snapshot_renders_real_rows() {
    let event = progressive(3);
    let window = reduce_feed_window(
        empty_feed_window(1, 180),
        FeedWindowEvidence::Snapshot {
            generation: 1,
            snapshot: snapshot(vec![event]),
            flags: FeedWindowFlags::default(),
        },
    );
    let model = build_notifications_feed_view(input(window));
...
fn snapshot(events: Vec<ProgressiveEvent>) -> ProgressiveReadSnapshot {
    ProgressiveReadSnapshot {
        read_id: "notifications-read".to_owned(),
...
fn progressive(value: u64) -> ProgressiveEvent {
    ProgressiveEvent {
        ...
        event: NostrEvent {
            ...
            content: format!("relay notification {value}"),
            sig: "b".repeat(128),
        },
    }
}
