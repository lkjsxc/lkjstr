use leptos::prelude::*;
use lkjstr_app::{FEED_LOAD_OLDER_COMMAND, FeedFooterRow, FeedFooterState, ThreadOlderLoadTrigger};
...
fn command_footer(
    row_id: String,
    text: &'static str,
    command: Callback<ThreadOlderLoadTrigger>,
) -> impl IntoView {
    let load_older = move |_| command.run(ThreadOlderLoadTrigger::Explicit);
    view! {
        <footer class="lkjstr-feed-footer" data-row-id=row_id>
            <button type="button" data-testid="thread-load-older" on:click=load_older>
                {text}
            </button>
        </footer>
    }
