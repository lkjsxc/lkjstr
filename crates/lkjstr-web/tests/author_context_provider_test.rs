...
async fn rust_author_context_tab_loads_cached_rows_from_host_provider() -> Result<(), JsValue> {
    reset_shells()?;
    clear_legacy()?;
    let db_name = test_db_name("author-context-provider");
    if let Err(error) = seed_author_context_cache(&db_name).await {
    ...
    wait_for_text("cached nearby author event").await?;
    wait_for_text("relay reads remain open").await
}

async fn seed_author_context_cache(db_name: &str) -> Result<(), JsValue> {
    ...
    put_event(&store, nearby_event()).await?;
...
fn relay_set() -> RelaySet {
    RelaySet {
        id: "author-context-test-relays".to_owned(),
        name: "Author Context Test Relays".to_owned(),
...
fn nearby_event() -> NostrEvent {
    event(2, 1_700_000_000, "cached nearby author event")
}
