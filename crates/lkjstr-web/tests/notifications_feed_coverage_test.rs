...
use lkjstr_protocol::{
    ...
    KIND_ZAP_RECEIPT, NostrEvent, NostrTag, normalize_relay_url,
};
use lkjstr_storage::{
    FeedCoverageRecord, NotificationRecord, StorageOutcome, StoredEventRecord,
    sqlite_event_relay_row,
};
use lkjstr_web::sqlite_store::{
    sqlite_account_put, sqlite_event_put, sqlite_feed_coverage_put, sqlite_notifications_put,
    sqlite_relay_set_put,
};
...
async fn rust_notifications_tab_uses_exact_account_coverage() -> Result<(), JsValue> {
    reset_shells()?;
    clear_legacy()?;
    let db_name = test_db_name("notifications-coverage");
    ...
    wait_for_text("Notifications ready").await?;
    wait_for_text("mention").await?;
    wait_for_text("covered notification event").await?;
...
async fn seed_notifications_cache(db_name: &str) -> Result<(), JsValue> {
    ...
    let relay_url = relay_set
        ...
        .url
        .clone();
    let normalized_relay = normalize_relay_url(&relay_url).ok_or_else(|| js_error("bad relay"))?;
    ...
    assert_ok(
        sqlite_feed_coverage_put(&store, &[coverage(&normalized_relay, &account.pubkey)]).await,
    )?;
...
fn coverage(relay_url: &str, pubkey: &str) -> FeedCoverageRecord {
    FeedCoverageRecord {
        coverage_id: "notifications-complete".to_owned(),
        feed_key: "notifications:rust-notifications-1".to_owned(),
        route_group_key: "selected:fallback".to_owned(),
        relay_url: relay_url.to_owned(),
        filter_fingerprint: notifications_filter_key(pubkey),
...
fn notifications_filter_key(pubkey: &str) -> String {
    format!(
        r#"{{"kinds":[{KIND_METADATA},{KIND_TEXT_NOTE},{KIND_REPOST},{KIND_REACTION},{KIND_GENERIC_REPOST},{KIND_ZAP_RECEIPT}],"tags":{{"p":["{pubkey}"]}}}}"#
    )
}

fn notification(owner_pubkey: &str, event: &NostrEvent) -> NotificationRecord {
    NotificationRecord {
        notification_id: "notification-coverage".to_owned(),
