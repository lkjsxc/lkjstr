...
mod home_feed_notification_shell_model;
mod home_feed_profile_hydration_dom;

use std::sync::{Arc, Mutex};

use accounts_selector_test_support::{click, next_task, reset_shells, wait_for_text};
use home_feed_notification_shell_model::{
    ANCHOR_EVENT_VALUE, dematerialized_model, initial_model, notification_row_id, startup,
};
...
async fn rust_home_notification_shell_preserves_measured_reservation() -> Result<(), JsValue> {
    reset_shells()?;
    ...
    wait_for_text("mention notification carrier").await?;
    ...
    let notification = observed_notification_row()?;
    let measured_height = wait_for_observed_height(&notification, 90).await?;
    let rendered_height = notification.offset_height().max(1);
    let anchor = event_row(ANCHOR_EVENT_VALUE)?;
    owner.set_scroll_top(
        relative_top(&owner, &notification)
            .saturating_add(rendered_height)
            .saturating_add(1),
    );
    next_task().await?;
    if relative_top(&owner, &notification) + rendered_height > owner.scroll_top() {
        return Err(js_error(
            "notification shell carrier row was not above viewport",
        ));
    }
    ...
    if shell.offset_height() + 1 < measured_height {
        return Err(js_error(
            "notification shell collapsed measured reservation",
        ));
    }
    Ok(())
}
...
impl CapturedHomeRequest {
    ...
    fn request(&self) -> Result<HomeFeedRequest, JsValue> {
        let guard = self
            .inner
            .lock()
            .map_err(|_| js_error("Home notification shell request capture poisoned"))?;
        guard
            .as_ref()
            .cloned()
            .ok_or_else(|| js_error("missing Home notification shell request"))
    }
}
...
async fn wait_for_observed_height(
    ...
    Err(js_error("notification shell carrier row was not measured"))
}
...
async fn wait_for_shell_anchor(
    ...
    Err(js_error("notification shell did not keep reserved height"))
}
...
fn observed_notification_row() -> Result<web_sys::HtmlElement, JsValue> {
    notification_row()?
        .closest(".lkjstr-feed-row-observer")?
        .ok_or_else(|| js_error("missing notification shell observer row"))?
        .dyn_into::<web_sys::HtmlElement>()
        .map_err(|_| js_error("notification shell observer row is not an HtmlElement"))
}

fn notification_row() -> Result<web_sys::HtmlElement, JsValue> {
    required_element(&format!(
        ".lkjstr-feed-row.notification[data-row-id='{}']",
        notification_row_id()
    ))
}
...
fn shell_row() -> Result<Option<web_sys::HtmlElement>, JsValue> {
    ...
    element
        .dyn_into::<web_sys::HtmlElement>()
        .map(Some)
        .map_err(|_| js_error("notification shell is not an HtmlElement"))
}
