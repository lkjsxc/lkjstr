...
async fn followees_island_mounts_and_unmounts() -> Result<(), JsValue> {
    reset_shells()?;
    let host = host_element()?;
    body()?.append_child(&host)?;

    let mut handle = lkjstr_web::mount_followees_tab(
        host.clone(),
        "followees-island-test".to_owned(),
        String::new(),
        no_op(),
        no_op(),
        no_op(),
    ...
    wait_for_text("Followees target unavailable.").await?;
    handle.unmount();
    next_task().await?;
    assert!(!host_text(&host).contains("Followees target unavailable."));
    host.remove();
    Ok(())
}
