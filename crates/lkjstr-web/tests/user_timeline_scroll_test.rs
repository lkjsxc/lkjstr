...
async fn rust_user_timeline_tab_uses_one_scroll_owner_for_header_status_and_rows()
-> Result<(), JsValue> {
    reset_shells()?;
    clear_legacy()?;
    let db_name = test_db_name("user-timeline-scroll-owner");
    ...
    assert_user_timeline_scroll_owner()
}

fn assert_user_timeline_scroll_owner() -> Result<(), JsValue> {
    assert_feed_scroll_boundary(
        ".lkjstr-user-timeline-feed",
        ".user-timeline-list-scroll[data-scroll-owner]",
        &[
            ".profile-card",
            ".lkjstr-feed-status",
            ".lkjstr-feed-rows",
    ...
    assert_tab_body_not_scroll_owner(".lkjstr-tab-body[data-tab-kind='user-timeline']")
}

async fn open_user_timeline_tab() -> Result<(), JsValue> {
