...
mod home_feed_lod_shell_model;
mod home_feed_profile_hydration_dom;

use std::sync::{Arc, Mutex};

use accounts_selector_test_support::{click, next_task, reset_shells, wait_for_text};
use home_feed_lod_shell_model::{
    ANCHOR_EVENT_VALUE, SHELL_EVENT_VALUE, dematerialized_model, initial_model, startup,
};
...
async fn rust_home_lod_shell_preserves_measured_reservation() -> Result<(), JsValue> {
    reset_shells()?;
    ...
    wait_for_text("lod shell carrier").await?;
    wait_for_text("visible anchor event").await?;

    let owner = home_scroll_owner()?;
    owner.set_attribute(
        "style",
        "display:block;height:112px;width:240px;overflow-y:auto;overflow-anchor:none;",
    )?;
    let carrier = observed_row(SHELL_EVENT_VALUE)?;
    let measured_height = wait_for_observed_height(&carrier, 140).await?;
    let anchor = event_row(ANCHOR_EVENT_VALUE)?;
    owner.set_scroll_top(
        relative_top(&owner, &carrier)
            .saturating_add(measured_height)
            .saturating_add(1),
    );
    next_task().await?;
    let before_top = owner.scroll_top();
    if relative_top(&owner, &carrier) + measured_height > before_top {
        return Err(js_error("LOD carrier row was not above viewport"));
    }
    ...
    if shell.offset_height() + 1 < measured_height {
        return Err(js_error("LOD shell collapsed measured reservation"));
    }
    Ok(())
}
...
impl CapturedHomeRequest {
    ...
    fn request(&self) -> Result<HomeFeedRequest, JsValue> {
        let guard = self
            .inner
            .lock()
            .map_err(|_| js_error("Home LOD request capture poisoned"))?;
        guard
            .as_ref()
            .cloned()
            .ok_or_else(|| js_error("missing Home LOD provider request"))
    }
}
...
async fn wait_for_observed_height(
    ...
    Err(js_error("LOD carrier row was not measured"))
}
...
async fn wait_for_shell_anchor(
    ...
    Err(js_error("LOD shell did not keep reserved height"))
}
...
fn observed_row(value: u64) -> Result<web_sys::HtmlElement, JsValue> {
    event_row(value)?
        .closest(".lkjstr-feed-row-observer")?
        .ok_or_else(|| js_error("missing LOD observer row"))?
        .dyn_into::<web_sys::HtmlElement>()
        .map_err(|_| js_error("LOD observer row is not an HtmlElement"))
}

fn shell_row() -> Result<Option<web_sys::HtmlElement>, JsValue> {
    let Some(element) = document()?.query_selector(".lkjstr-feed-row.lod-shell[data-lod-shell]")?
    else {
        return Ok(None);
    };
    element
        .dyn_into::<web_sys::HtmlElement>()
        .map(Some)
        .map_err(|_| js_error("LOD shell is not an HtmlElement"))
}
