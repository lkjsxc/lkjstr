...
async fn profile_island_mounts_and_unmounts() -> Result<(), JsValue> {
    reset_shells()?;
    let host = host_element()?;
    body()?.append_child(&host)?;

    let mut handle = lkjstr_web::mount_profile_tab(
        host.clone(),
        "profile-island-test".to_owned(),
        String::new(),
        String::new(),
        action_object()?,
    )?;

    wait_for_text("Profile unavailable").await?;
    handle.unmount();
    next_task().await?;
    assert!(!host_text(&host).contains("Profile unavailable"));
    host.remove();
    Ok(())
}
...
fn action_object() -> Result<JsValue, JsValue> {
    ...
    for name in [
        ...
        Reflect::set(actions.as_ref(), &JsValue::from_str(name), no_op().as_ref())?;
    }
    Ok(actions.into())
}
