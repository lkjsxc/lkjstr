...
async fn home_island_mounts_and_unmounts() -> Result<(), JsValue> {
    reset_shells()?;
    let host = host_element()?;
    body()?.append_child(&host)?;

    let mut handle = lkjstr_web::mount_home_tab(
        host.clone(),
        "home-island-test".to_owned(),
        String::new(),
        no_op(),
        no_op(),
        no_op(),
    ...
    handle.unmount();
    next_task().await?;
    assert!(!host_text(&host).contains("No active account"));
    host.remove();
    Ok(())
}
