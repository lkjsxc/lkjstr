...
use lkjstr_app::{
    EventDisplayContext, EventDisplayInput, FeedFooterState, FeedShellRow, FeedViewModel,
    FeedViewRow, GeometryEstimateSource, HomeFeedStatus, HomeFeedView, RowGeometryEstimate,
    StartupInput, default_recovery_ids, feed_event_row_id, footer_row, plan_event_display,
};
use lkjstr_protocol::KIND_TEXT_NOTE;

pub const SHELL_EVENT_VALUE: u64 = 40;
...
pub fn initial_model() -> HomeFeedView {
    HomeFeedView {
        ...
        view_model: FeedViewModel {
            feed_id: "home-lod-shell".to_owned(),
            rows: vec![
                text_event(SHELL_EVENT_VALUE, &shell_carrier_text()),
                text_event(ANCHOR_EVENT_VALUE, "visible anchor event"),
                text_event(10, &"below shell anchor filler ".repeat(90)),
                FeedViewRow::Footer(footer_row("home-lod-shell", FeedFooterState::CacheHit)),
            ],
        },
    }
}
...
fn shell_row(reserved_height_px: u16) -> FeedViewRow {
    let row_id = feed_event_row_id(&id(SHELL_EVENT_VALUE));
...
fn shell_carrier_text() -> String {
    "lod shell carrier preserves measured height ".repeat(90)
}
