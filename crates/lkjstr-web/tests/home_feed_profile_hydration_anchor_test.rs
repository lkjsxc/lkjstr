...
mod home_feed_profile_hydration_model;
...
use home_feed_profile_hydration_model::{CapturedHomeRequest, hydrated_profile_name, startup};
...
async fn rust_home_profile_hydration_preserves_visible_anchor() -> Result<(), JsValue> {
    reset_shells()?;
    ...
    owner.set_attribute(
        "style",
        "display:block;height:96px;width:260px;overflow-y:auto;overflow-anchor:none;",
    )?;
    next_task().await?;
    ...
    if before_top <= 0 {
        return Err(js_error(
            "Home anchor setup did not scroll below hydrated row",
        ));
    }

    captured.request()?.complete(hydrated_profile_name());
    wait_for_text("hydrated-profile-tail").await?;
    next_task().await?;

    if owner.scroll_top() <= before_top {
        return Err(js_error(
            "profile hydration above viewport did not move scrollTop",
        ));
    }
