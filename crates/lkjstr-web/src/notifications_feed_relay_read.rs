...
use crate::{
    ...
    notifications_feed_host::WINDOW_MAX,
    notifications_feed_relay_input::NotificationsRelayReadInput,
    notifications_feed_relay_input::notification_event_matches_read,
    notifications_feed_relay_model::NotificationsRelayReadOutput,
    notifications_feed_relay_read_tail::connect_socket,
...
pub(crate) fn start_read(
    ...
    let read = Rc::new(NotificationsRelayRead {
        input,
        sub_id: sub_id.clone(),
        filters,
    ...
    let handle = RelayReadHandle::from_rc(&read, NotificationsRelayRead::cancel);
...
pub(super) struct NotificationsRelayRead {
    pub(super) input: NotificationsRelayReadInput,
    ...
    pub(super) complete: Box<dyn Fn(NotificationsRelayReadOutput)>,
}

fn install_timeout(read: Rc<NotificationsRelayRead>) {
    if read.done.get() {
        return;
    }
...
impl NotificationsRelayRead {
    ...
    fn event(&self, relay: &str, event: lkjstr_protocol::NostrEvent) {
        ...
        if !notification_event_matches_read(&self.input, &event) {
            return;
        }
