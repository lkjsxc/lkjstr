...
use crate::{
    ...
    thread_feed_host::WINDOW_MAX,
    thread_feed_relay_input::{ThreadRelayReadInput, thread_event_matches_read},
    thread_feed_relay_read_tail::connect_socket,
};
...
pub(crate) fn start_read(
    ...
    relays: Vec<String>,
    complete: impl Fn(crate::thread_feed_relay_model::ThreadRelayReadOutput) + 'static,
) -> RelayReadHandle {
    ...
    let handle = RelayReadHandle::from_rc(&read, ThreadRelayRead::cancel);
...
pub(super) struct ThreadRelayRead {
    pub(super) input: ThreadRelayReadInput,
    ...
    pub(super) complete: Box<dyn Fn(crate::thread_feed_relay_model::ThreadRelayReadOutput)>,
}

fn install_timeout(read: Rc<ThreadRelayRead>) {
    if read.done.get() {
        return;
    }
...
impl ThreadRelayRead {
    ...
    fn event(&self, relay: &str, event: lkjstr_protocol::NostrEvent) {
        if self.done.get() || !thread_event_matches_read(&self.input, &event) {
            return;
        }
