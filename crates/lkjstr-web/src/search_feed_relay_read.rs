...
use crate::{
    ...
    search_feed_host::WINDOW_MAX,
    search_feed_relay_input::{SearchRelayReadInput, search_event_matches_read},
    search_feed_relay_read_tail::connect_socket,
};
...
pub(crate) fn start_read(
    ...
    let read = Rc::new(SearchRelayRead {
        input,
        sub_id: sub_id.clone(),
        filters,
    ...
    let handle = RelayReadHandle::from_rc(&read, SearchRelayRead::cancel);
...
pub(super) struct SearchRelayRead {
    pub(super) input: SearchRelayReadInput,
    ...
    pub(super) complete: Box<dyn Fn(SearchFeedView)>,
}

fn install_timeout(read: Rc<SearchRelayRead>) {
    if read.done.get() {
        return;
    }
...
impl SearchRelayRead {
    ...
    fn event(&self, relay: &str, event: lkjstr_protocol::NostrEvent) {
        ...
        if !search_event_matches_read(&self.input, &event) {
            return;
        }
