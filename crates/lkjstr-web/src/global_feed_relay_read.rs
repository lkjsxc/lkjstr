...
use crate::{
    global_feed_host::WINDOW_MAX,
    global_feed_relay_input::{GlobalRelayReadInput, global_event_matches_read},
    global_feed_relay_model::GlobalRelayReadOutput,
    global_feed_relay_read_tail::connect_socket,
...
pub(crate) fn start_read(
    ...
    let read = Rc::new(GlobalRelayRead {
        input,
        sub_id: sub_id.clone(),
        filters,
    ...
    let handle = RelayReadHandle::from_rc(&read, GlobalRelayRead::cancel);
...
pub(super) struct GlobalRelayRead {
    pub(super) input: GlobalRelayReadInput,
    ...
    pub(super) complete: Box<dyn Fn(GlobalRelayReadOutput)>,
}

fn install_timeout(read: Rc<GlobalRelayRead>) {
    if read.done.get() {
        return;
    }
...
impl GlobalRelayRead {
    ...
    fn event(&self, relay: &str, event: lkjstr_protocol::NostrEvent) {
        ...
        if !global_event_matches_read(&self.input, &event) {
            return;
        }
