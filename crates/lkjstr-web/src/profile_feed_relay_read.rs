...
use crate::{
    ...
    profile_feed_host::WINDOW_MAX,
    profile_feed_relay_input::{ProfileRelayReadInput, profile_event_matches_read},
    profile_feed_relay_read_tail::connect_socket,
...
pub(crate) fn start_read(
    ...
    let read = Rc::new(ProfileRelayRead {
        input,
        sub_id: sub_id.clone(),
        filters,
    ...
    let handle = RelayReadHandle::from_rc(&read, ProfileRelayRead::cancel);
...
pub(super) struct ProfileRelayRead {
    pub(super) input: ProfileRelayReadInput,
...
fn install_timeout(read: Rc<ProfileRelayRead>) {
    if read.done.get() {
        return;
    }
...
impl ProfileRelayRead {
    ...
    fn event(&self, relay: &str, event: lkjstr_protocol::NostrEvent) {
        if self.done.get() || !profile_event_matches_read(&self.input, &event) {
            return;
        }
