<script lang="ts">
  ...
  import { publishTweet, tweetPublishFailureMessage } from '$lib/tweet/publish';
  import {
    clearTweetDraft,
  ...
  async function publish(): Promise<void> {
    ...
    try {
      ...
      sensitive = false;
      warningReason = '';
      message = result.archiveWarning ?? '';
      await Promise.all([clearTweetDraft(draftId), clearTweetDraft('main')]);
      await tick();
      if (destroyed) return;
      focusNonce += 1;
      // prettier-ignore
      void result.delivery.then((results) => { if (!destroyed && results.every((item) => !item.accepted)) message = 'All relays rejected the event.'; }).catch((error) => { if (!destroyed) message = tweetPublishFailureMessage(error); });
    } catch (error) {
      if (!destroyed) message = tweetPublishFailureMessage(error);
    } finally {
      if (!destroyed) publishing = false;
    }
  }
