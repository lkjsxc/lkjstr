...
- Followees/User Timeline mount Rust bodies through Svelte as WASM islands and
  ...
  partial-route, and degraded states. Rust Followees rows open Profile and keep Timeline/Copy npub in overflow with explicit copy status.
...
- Rust owns pure feed row geometry estimates, reservation decisions, anchor
  ...
  User Timeline use durable models; converted rebuilds retain them. Stats shows row-height counts; runtime counters remain open. Reservation rules live in
