...
- Publishing requires an active local or NIP-07 signing account.
...
- `Ctrl+Enter` in the editor uses the same disabled rules as the Publish
  button.
- Paste, bundled image presets, and file-picker uploads use the configured
  NIP-96 HTTPS upload server.
