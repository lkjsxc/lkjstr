...
- Feed-runtime SQLite wiring, pressure plus byte inventory diagnostics, Search
  app planning, and NIP-50 merge. Retention delete dispatch, repair worker
...
- Current sub-slice: Stats consumes real SQLite worker health through the Rust
  ...
  byte-summary Stats rows, and Search token/tag/query metadata are implemented;
  localStorage, Cache Storage, old IndexedDB Rust host inventory, full product
