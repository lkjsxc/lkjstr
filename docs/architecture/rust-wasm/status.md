...
- Relay adapter product wiring from pure reducers to browser WebSocket and timer
  handles.
- Feed-runtime SQLite wiring, repair paths, search/tag command metadata, and
  pressure plus byte inventory diagnostics. Retention delete dispatch is wired
  at the Rust adapter boundary, but product consumption remains open.
...
- Current sub-slice: Stats consumes real SQLite worker health through the Rust
  ...
  metadata, retention planner metadata, and retention delete dispatch are
  implemented; repair, search/tag lookup, full product cache proof, and surface
