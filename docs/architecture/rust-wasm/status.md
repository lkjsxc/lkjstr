...
- Current sub-slice: Stats consumes real SQLite worker health through the Rust
  ...
  byte-summary Stats rows, localStorage count/status, and Search token/tag/query
  metadata are implemented; localStorage byte estimates, Cache Storage, old
  IndexedDB Rust host inventory, full product cache proof, Search app planning,
