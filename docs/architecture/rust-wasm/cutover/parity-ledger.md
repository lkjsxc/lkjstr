...
| Custom Request  | partial         | `app`, `protocol`, `relays`, `ui` request runtime             | host relay output, cancellation, Leptos states, no-import proof                              |
...
buttons are wired, and browser proof covers explicit unavailable states.
