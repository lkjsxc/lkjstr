...
| Author Context  | partial         | `app`, `web`, `relays`, `storage`, `ui` context runtime       | cache and relay reads, action opening, unavailable states, no-import proof                   |
...
| Author Context  | Injected rows, cache reads, relay reads, action opening, unavailable states, and no-import proof |
