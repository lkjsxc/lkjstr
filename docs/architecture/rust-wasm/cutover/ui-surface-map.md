...
- `TabBody` renders real Leptos Welcome, New Tab, Stats, Log, Accounts, Relay
  ...
  worker/relay-backed Author Context shared-feed rows and row actions. Other
  tab kinds still render pending Leptos bodies while shipped behavior remains
  Svelte.
...
| `src/lib/tabs/author-context/**`, `src/lib/author-context/**`                                  | `workspace/author_context*.rs`                                 | `lkjstr-app`, `lkjstr-web`, `lkjstr-relays`, `lkjstr-storage`      | Exact lookup, stored routes, unavailable states, no-import proof, and final gate pass.           |
