...
| `src/lib/identity`, `src/lib/author-context`                                        | domain-logic and view-only             | identity cache, NIP-05 state, Author Context tab            | `lkjstr-app`, `lkjstr-storage`, `lkjstr-ui`                       | Rust identity resolution and context runtime ship                       |
...
| `src/lib/search`                                                                    | feed-logic and storage-logic           | shipped SQLite token index and query helpers                | `lkjstr-app`, `lkjstr-storage`, `lkjstr-relays`                   | Rust local query adapter, app planner, NIP-50 merge, and UI parity land |
| `src/lib/custom-request`                                                            | protocol-logic and relay-logic         | shipped request runner and validation display               | `lkjstr-app`, `lkjstr-protocol`, `lkjstr-relays`                  | Rust parser drives selected-relay request effects                       |
