...
records, the tab-state key plus ledger-row contract for workspace snapshots,
and Rust record shapes for protected startup data. `lkjstr-web` owns a narrow
real IndexedDB adapter for workspace startup, workspace rows, settings override
rows, protected account rows, local secrets, relay sets, Tweet drafts, and the
first multi-store transaction helper. Rust tab-state snapshot writes store the
`tabStates` row and matching `cacheLedger` row in one IndexedDB transaction.
