...
| ------- | --------------------- | ----------------- | ------------------- | --------------------------- | --------------- | -------- |
| Home | implemented through timeline page reads | open as primary product path | open for product span choice | current hints and coverage now; density rows open | durable rows visible when present | selected relays and current span policy |
| Global | implemented through timeline page reads | open as primary product path | open for product span choice | current hints and coverage now; density rows open | durable rows visible when present | selected relays and current span policy |
| Profile posts | implemented through profile route plans | open as primary product path | open for product span choice | current hints and coverage now; density rows open | durable rows visible when present | selected relays plus route groups |
| Notifications | implemented for older notification feed pages | open as primary product path | open for product span choice | current hints and coverage now; density rows open | durable rows visible when present | selected notification relays |
...
- Transient tab ids, pane ids, owner handles, request ids, and subscription ids
  stay out of scan model keys.
