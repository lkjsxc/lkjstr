...
cached rows consume matching models first. Full Leptos feed use remains an active target.
...
10. Load matching model rows into converted feed hosts before estimating rows;
    ...
    Timeline cached rows are first, and broader host coverage remains open.
