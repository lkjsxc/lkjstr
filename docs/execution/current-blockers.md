...
- Completed enabling proof: `lkjstr-app` now has a pure row view model with
  ...
  planning with fallback open-link identity attributes, unavailable reference
  rows with real referenced event identity and relay-hint UI attributes, inline
  content event-reference token suppression before Rust row planning, safe HTTPS
  link rows with inline embedded-media URL suppression,
