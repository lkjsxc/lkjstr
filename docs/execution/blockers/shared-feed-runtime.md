...
- Completed enabling proof: `lkjstr-app` now has a pure row view model with
  ...
  Docker final-gate proof, typed Home/Notifications read-availability fallback
  proof, Home cache-unavailable follow discovery with real relay-row recovery,
  and `CONNECTING` relay socket cancellation proof. Generic feed-host selection now has focused
